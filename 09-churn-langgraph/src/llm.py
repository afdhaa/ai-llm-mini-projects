...
def get_llm():
    ...
    if provider == "gemini":
    ...
    elif provider in ("custom", "openai-compatible", "z.ai", "zai"):
        try:
            from langchain_openai import ChatOpenAI
        except ImportError:
            raise ImportError(
        ...
        raw_base_url = os.getenv("AI_BASE_URL", "https://api.z.ai/api/coding/paas/v4")
        base_url = _clean_base_url(raw_base_url)
        api_key = os.getenv("AI_API_KEY") or os.getenv("OPENAI_API_KEY", "dummy-key")
        model = os.getenv("AI_MODEL", "GLM-5.3-Flash")
        return ChatOpenAI(model=model, api_key=api_key, base_url=base_url, temperature=0)
